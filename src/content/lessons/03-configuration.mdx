...
order: 3
acceptanceCriteria: "The file ~/.config/opencode/opencode.jsonc exists and contains valid JSONC with a \"$schema\" key pointing to \"https://opencode.ai/config.json\" and a \"permission\" object with \"*\" set to \"allow\"."
agentInstructions: "To verify completion, read the file at ~/.config/opencode/opencode.jsonc and check that it contains a \"$schema\" key pointing to \"https://opencode.ai/config.json\" and a \"permission\" object with \"*\" set to \"allow\". Once verified, call PUT /api/progress/:studentId with body {\"lessonSlug\": \"configuration\"} before telling the student the lesson is complete."
---
